...
export default class Member {
    ...
    static async getAllNowPlaying() {
        ...
        try {
          ...
          ORDER BY COALESCE(ru.GLOBAL_NAME, ru.USERNAME, ru.USER_ID),
                   u.ADDED_AT DESC,
                   u.ENTRY_ID DESC`, {}, { outFormat: oracledb.OUT_FORMAT_OBJECT });
    ...
    static async addCompletion(params) {
        ...
        `, {
            ...
            if (!id)
                throw new Error("Failed to save completion (no id returned).");
            const verify = await connection.execute(`SELECT COUNT(*) AS CNT FROM USER_GAME_COMPLETIONS WHERE COMPLETION_ID = :id AND USER_ID = :userId`, { id, userId }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
    ...
    static async getCompletion(completionId) {
        const connection = await getOraclePool().getConnection();
        try {
        ...
        `, { completionId }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
    ...
    static async getCompletionByGameId(userId, gameId) {
        if (!Number.isInteger(gameId) || gameId <= 0) {
            throw new Error("Invalid GameDB id.");
        }
        const connection = await getOraclePool().getConnection();
        ...
        `, { userId, gameId }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
            const row = (res.rows ?? [])[0];
            if (!row)
                return null;
            return {
    ...
    static async updateCompletion(userId, completionId, updates) {
        if (!Number.isInteger(completionId) || completionId <= 0) {
            throw new Error("Invalid completion id.");
        }
        const fields = [];
        const binds = { userId, completionId };
    ...
    static async deleteCompletion(userId, completionId) {
        if (!Number.isInteger(completionId) || completionId <= 0) {
            throw new Error("Invalid completion id.");
        }
        const connection = await getOraclePool().getConnection();
        ...
        DELETE FROM USER_GAME_COMPLETIONS
         WHERE COMPLETION_ID = :completionId
           AND USER_ID = :userId
        `, { completionId, userId }, { autoCommit: true });
            const rows = res.rowsAffected ?? 0;
            return rows > 0;
        }
        finally {
