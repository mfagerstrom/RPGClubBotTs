...
function getSessionStore() {
    ...
    if (!g[IGDB_SESSION_KEY]) {
        g[IGDB_SESSION_KEY] = new Map();
    }
    return g[IGDB_SESSION_KEY];
}
function chunkOptions(options, page) {
...
export function buildIgdbComponents(sessionId, page) {
...
export function deleteIgdbSession(sessionId) {
...
function resolveIgdbSelection(sessionId, page, value) {
    const session = getSessionStore().get(sessionId);
    if (!session)
        return null;
