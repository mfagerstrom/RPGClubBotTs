...
async function handleLinkButton(interaction, threadId) {
    ...
    try {
        ...
        const localMatches = (await Game.searchGames(title)).filter((g) => g.title.toLowerCase() === title.toLowerCase());
        let gameId = localMatches[0]?.id ?? null;
        let chosenName = localMatches[0]?.title ?? null;
        const finalizeSelection = async (igdbId, nameHint) => {
        ...
        if (!gameId) {
            ...
            const selectFirst = async (igdbId, name) => {
...
export function startThreadLinkPromptService(client) {
    if (!hasIgdbConfig()) {
        console.warn("[ThreadLinkPrompt] IGDB_CLIENT_ID/SECRET not set; skipping thread link prompts.");
    }
    ...
    client.on("messageCreate", async (message) => {
        ...
        if (!("isThread" in channel) || !channel.isThread())
            return;
        if (channel.parentId !== NOW_PLAYING_FORUM_ID)
            return;
