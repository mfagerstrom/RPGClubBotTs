...
let RemindMeCommand = class RemindMeCommand {
    ...
    async snooze(reminderId, until, interaction) {
        ...
        const updated = await Reminder.snooze(reminderId, interaction.user.id, remindAt.toJSDate());
        if (!updated) {
            await safeReply(interaction, {
...
let RemindMeButtons = class RemindMeButtons {
    async handleReminderButtons(interaction) {
        ...
        const updated = await Reminder.snooze(reminder.reminderId, interaction.user.id, newTime.toJSDate());
        if (!updated) {
            await safeReply(interaction, {
...
], RemindMeButtons.prototype, "handleReminderButtons", null);
RemindMeButtons = __decorate([
    Discord()
], RemindMeButtons);
export { RemindMeButtons };
