...
function buildAvatarDataUrl(record, user) {
...
export function buildProfileHtml(record, user) {
    ...
    const fieldsHtml = items
        .map((item) => `<div class="field"><div class="label">${escapeHtml(item.label)}</div><div class="value">${item.value}</div></div>`)
        .join("");
    return `<!DOCTYPE html>
<html>
  <head>
  ...
  <body>
    <div class="card">
      <div class="header">
        ...
        <div>
          <h1 class="title">${escapeHtml(record.globalName ?? record.username ?? user.username ?? "Member")}</h1>
...
export async function renderProfileImage(record, user) {
