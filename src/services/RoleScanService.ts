...
type MemberScanResult = { successCount: number; failCount: number };

export async function memberScanTick(
  client: Client,
  opts?: { logChannelId?: string; throttleMs?: number },
): Promise<MemberScanResult> {
  ...
  if (logChannelId) {
    ...
    logChannel = fetched && fetched.isTextBased() ? (fetched as TextBasedChannel) : null;
  }
  ...
  for (const guild of guilds.values()) {
    ...
    for (const member of members.values()) {
      ...
      const moderatorFlag =
        hasRole(ROLE_IDS.mod) || member.permissions.has("ManageMessages") ? 1 : 0;
      ...
      if (logChannel && isSendable(logChannel)) {
        ...
        if (oldNick !== newNick) {
          const embed = new EmbedBuilder()
            ...
            .addFields(
              { name: "Old", value: oldNick ?? "(none)", inline: true },
              { name: "New", value: newNick ?? "(none)", inline: true },
            )
        ...
        if (avatarBuffersDifferent(existingRow?.AVATAR_BLOB ?? null, avatarBlob)) {
          ...
          let newImg: AttachmentBuilder | null = null;
          let oldImg: AttachmentBuilder | null = null;
          if (avatarBlob) {
